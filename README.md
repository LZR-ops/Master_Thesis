...
lightweight topic modeling on negative feedback using BERTopic.

Key findings. The fine-tuned DistilBERT model achieved the best performance with 66.90% accuracy, significantly outperforming the VADER baseline (55.14%) and traditional machine learning models (up to 64.90%). The model showed particularly strong capability in detecting negative sentiment, which is critical for brand reputation management. The developed dashboard enables real-time sentiment
analysis and extraction of key topics from negative comments, providing actionable insights for businesses.
